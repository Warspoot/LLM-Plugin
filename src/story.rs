...
unsafe fn read_il2cpp_string(s: *mut api::Il2CppString) -> String {
    if s.is_null() {
        return String::new();
    }
...
pub fn poll() {
    ...
    crate::logging::info(&format!("story::poll: BlockList has {count} blocks"));
    ...
    crate::logging::info(&format!("story::poll: block {i} text = {text:?}"));
}
